# Modifying the code in code_9.py and make it asyncronous for better performance
...
ORIGINAL_DIR = Path(r"D:\Desktop\Python_Programs\Lesson 32 AsyncIO\img\raw\demo1")
PROCESSED_DIR = Path(r"D:\Desktop\Python_Programs\Lesson 32 AsyncIO\img\processed\demo1")


async def download_single_image(url: str, img_num: int) -> Path:
    print(f"Downloading {url}...")
    # Run blocking code in thread pool
    return await asyncio.to_thread(_download_single_image, url, img_num)


def _download_single_image(url: str, img_num: int) -> Path:
    ts = int(time.time())
...
async def download_images(urls: list) -> list[Path]:
    ...
    async with asyncio.TaskGroup() as tg:
        for img_num, url in enumerate(urls, start=1):
            tg.create_task(download_single_image(url, img_num))
    # Results are printed in the function itself
    return []
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    ...
    async with asyncio.TaskGroup() as tg:
        for orig_path in orig_paths:
            tg.create_task(process_single_image(orig_path))
    # Results are printed in the function itself
    return []


async def process_single_image(orig_path: Path) -> Path:
    # Run blocking code in thread pool
    return await asyncio.to_thread(_process_single_image, orig_path)


def _process_single_image(orig_path: Path) -> Path:
...
async def main():
    ...
    # Get downloaded image paths from directory
    img_paths = list(ORIGINAL_DIR.glob("*.jpg"))

    proc_start_time = time.perf_counter()

    await process_images(img_paths)

    # Get processed image paths from directory
    processed_paths = list(PROCESSED_DIR.glob("*.jpg"))
